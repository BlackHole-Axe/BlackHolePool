#!/usr/bin/env bash
# ════════════════════════════════════════════════════════════════════════════
#  BlackHole Pool — Universal Installer
#
#  Supports:
...
#    7. Builds BlackHole with git provenance
...
if $ZMQ_OK; then
  ...
  if ! $UNATTENDED && [ -n "$BTC_CONF" ] && [ -f "$BTC_CONF" ]; then
    ...
    if [[ "$(echo "$ans" | tr '[:upper:]' '[:lower:]')" == y* ]]; then
      {
        printf '\n# ZMQ — added by BlackHole installer %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
cat > "env/.env" << ENVEOF
# ═══════════════════════════════════════════════════════════════════════════
#  BlackHole Pool — Runtime Configuration
...
POOL_TAG=/BlackHole Solo Pool
COINBASE_MESSAGE=BlackHole
...
if [ "$PLATFORM" = "umbrel" ]; then
  ...
  if [ ! -f "docker-compose.standalone.yml" ]; then
    ...
    err "Make sure you cloned/extracted the full BlackHole repo."
    exit 1
  fi

  # If Bitcoin Core is in Docker, generate a network-bridge override so the
  # pool container can reach it without needing published ports.
  if [ -n "$BTC_CONTAINER" ] && [ -n "${BTC_DOCKER_NET:-}" ] \
     && [ "$BTC_DOCKER_NET" != "bridge" ]; then
    info "Generating network bridge override for Bitcoin Core network…"
    cat > docker-compose.network-override.yml << NETEOF
# Auto-generated by setup-blackhole.sh — joins pool to Bitcoin Core's network
# This allows direct container-to-container communication without published ports.
services:
  blackhole-pool:
    networks:
      blackhole_net: {}
...
# ════════════════════════════════════════════════════════════════════════════
sep; info "Phase 9 — Building BlackHole"
...
# ════════════════════════════════════════════════════════════════════════════
sep; info "Phase 10 — Starting BlackHole"
...
conn_json=$(curl -sf --max-time 5 "$API/blackhole/connection-status" 2>/dev/null || echo "{}")
...
sep
echo ""
echo -e "${BLD}${GRN}  ✓ BlackHole Pool is running!${RST}"
echo ""
